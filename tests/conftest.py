...
def fake_case():
    ...
    return {
        ...
        "caseAttributes": {
            ...
            "caseDate": "2026-04-22T00:00:00",  # 5 days before fixed_now (2026-04-27)
        },
    }
...
def patch_external_functions(monkeypatch, fake_case, fake_party):
    ...
    # Baseline helper default: no reminders yet — anchor on the case's caseDate.
    monkeypatch.setattr(
        nf, "get_next_reminder_baseline",
        lambda case, access: (0, case.get("caseAttributes", {}).get("caseDate"), 14),
    )
